...
| `spine-all-executed.synthetic.vitest.json` | **Synthetic**, derived from the file above by flipping every skipped assertion to `passed`. Not observed output. It is the positive control: it proves the checker can return `PASS`, so a `FAIL` on the real report is a finding rather than a checker that can only ever fail. **Extended for RA-7753** with the six suites that change added to the manifest (`cec_separation`, `credential_provenance`, `identity_rules`, `rls_catalog`, `tenure_visibility`, `unit/ephemeral-guard`): their records are copied from a real local vitest JSON run against the loopback Supabase with only the path prefix rewritten, and the summary counts are recomputed from the records (73 tests, 30 suites). The six original records are unchanged. It must track the shipped manifest, because the positive control grades it against `resolveCheck(loadManifest(...))`. `spine-skipped.vitest.json` is deliberately NOT extended: it is a real historical run, and the new suites genuinely are absent from it. |
