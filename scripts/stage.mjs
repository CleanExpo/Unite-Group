...
export function summariseTeam(key, name, issues, capped, map) {
  ...
  // Same rule as project-stages.ts: open work the map cannot classify is never Done.
...
export async function fetchTeamStages({ apiKey, fetchImpl = fetch, map = loadStageMap(), now = () => new Date() } = {}) {
  ...
  for (const team of teams) {
    ...
    // More pages remain after the cap: an unseen issue could be further along. Fail (mirror of project-stages.ts).
    if (capped) throw new Error(`${team.key}: more than ${MAX_PAGES * PAGE_SIZE} open issues: the stage cannot be verified from a partial read`)
    // An open issue the map cannot classify has an unknown stage. Fail the read (mirror of project-stages.ts).
    const unknown = [...new Set(issues.filter((i) => classifyIssue(i, map) === null).map((i) => i.stateName))].sort()
    if (unknown.length > 0) {
      throw new Error(`${team.key}: open issues in states the stage map does not know (${unknown.join(', ')}); add them to data/command-centre/stage-map.json`)
    }
