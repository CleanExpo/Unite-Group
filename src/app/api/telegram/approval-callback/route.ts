...
function config() { const token = process.env.TELEGRAM_BOT_TOKEN; const signingKey = process.env.TELEGRAM_DECISION_SIGNING_KEY; if (!token || !signingKey) throw new Error('ERR_INTERNAL'); return { token, signingKey, gatePath: process.env.PI_APPROVAL_GATE_PATH ?? path.resolve(process.cwd(), 'docs/margot/personal-intelligence/approval-gate/2026-05-25-phase-1h-approval-gate-example.json'), ledgerPath: process.env.PI_DECISION_LEDGER_PATH ?? path.resolve(process.cwd(), 'docs/margot/personal-intelligence/approval-decisions/phase-1i-telegram-decisions.jsonl') }; }
