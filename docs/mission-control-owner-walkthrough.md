...
PR #1076 repaired a verified source weakness: classification, clarification and software planning depended on prompt-only JSON and read only the first content block. Strict preparation now requests schema-constrained output, reads the response's text blocks and rejects incomplete or refused responses while retaining local validation. Anthropic documents this contract and its exceptions in [Structured outputs](https://platform.claude.com/docs/en/build-with-claude/structured-outputs). Main `f20e1f80` reached production on deployment `dpl_JDfwS9s3yhL7qfrWi86NN3aiNgoD`; all twelve required PR checks and post-merge main CI passed.

Strict specification generation also needs room for both reasoning and the final JSON: [Sonnet 5's migration guide](https://platform.claude.com/docs/en/models/sonnet-5/migration-guide) confirms that adaptive thinking is on by default and shares the output limit. The repair sets a bounded 4,096-token ceiling for this step; it does not add retries or change model IDs.

At **07:19–07:20 UTC (17:19–17:20 AEST)**, the authenticated owner session continued the same mission, answered three business-context questions and generated a saved specification and AI Board review. Its title is **Draft Mission Control improvement spec: GitHub access fix & ownership dashboard**, with eight acceptance criteria, six delivery steps and specification version `5c244c64fee2a01c78fa9a6b0c17b2d3ed30de910ad071645e755aa4fc665159`. The Board returned **HOLD**, requesting evidence for the GitHub connection repair and repository access scope. The task is proposed, preparation is ready, and approval/build records remain null. No generated-mission build or release was started. The Board perspectives are model-generated advice, not evidence of independently accepted worker or human Board assignments.

That live result exposed a decision-order problem: the next step urged approval, while Board concerns appeared below the consent controls. The presentation repair moves the actual HOLD/REJECTED verdict and rationale before the specification and consent, asks the owner to review concerns before deciding, and explicitly states that branch consent does not resolve them. Other delivery blockers retain their own section. This preserves the existing approval payload and server behaviour.

**Governance gap:** delivery approval and runner admission currently do not enforce Board verdicts, although the Board prompt describes HOLD/REJECTED as gates. There is no established delivery-mission override policy or general same-mission revise/re-review action; ready-state resume is a no-op. A binding Board gate therefore needs an explicit policy and revision/re-review path together. This presentation repair does not claim to settle either.
...
Waterline: The owner mission has a saved specification and Board HOLD. No build or release has been approved or generated.
