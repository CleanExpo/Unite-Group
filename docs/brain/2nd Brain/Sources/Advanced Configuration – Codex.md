---
title: "Advanced Configuration – Codex"
...
Set `default_permissions` to reuse a sandbox profile by name. Codex includes the built-in profiles `:read-only`, `:workspace`, and `:danger-no-sandbox`:

```toml
default_permissions = ":workspace"
```

For custom profiles, point `default_permissions` at a name you define under `[permissions.<name>]`:
...
Use built-in names with a leading colon. Custom names don’t use a leading colon and must have matching `permissions` tables.
