...
- **Tools**: By default, the Antigravity agent has access to `code_execution`, `google_search`, and `url_context`. You can override this list by passing the `tools` parameter at interaction time. For full details on available tools and how to use them, see [Antigravity Agent: Supported tools](https://ai.google.dev/gemini-api/docs/antigravity-agent#supported-tools).
