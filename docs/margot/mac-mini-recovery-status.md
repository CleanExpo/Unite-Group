...
- Latest probe at `2026-05-23 16:11 AEST`: `/Volumes` still has no authenticated Mac Mini share mounted; `phills-mac-mini.local` SMB/File Sharing port `445` is currently unreachable; SSH/Remote Login port `22` is unreachable; `docs/margot/recovered-from-mac-mini/` still contains only `.gitkeep`.
