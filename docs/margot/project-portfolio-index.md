...
| AI enhancement pipeline | Unite-Group operating system | `docs/margot/ai-enhancement-pipeline.md`; `docs/margot/ai-enhancement-candidate-register.md`; Senior PM model defines watch/triage/sandbox/evaluate/adopt flow | Active local operating model plus candidate register. The register currently tracks deterministic lead qualification, daily digest, retrieval evaluation, integration stale-sync/risk summarization, and voice privacy/retention as source-labeled safe lanes; no new vendor or external AI enrichment is adopted. | 1. Keep the candidate register current as evaluations start. 2. Build local-only retrieval evaluation fixtures with expected source-file citations. 3. Keep deterministic CRM helpers ahead of probabilistic AI scoring until privacy/identity/approval gates are explicit. | No live AI vendor/tool evaluation is performed by this lane; client/lead data enrichment, transcript summarization, new vendor accounts, and production adoption remain approval-gated. | High: compounds automation, retrieval, QA, and operational leverage while preventing random tool chasing. |
