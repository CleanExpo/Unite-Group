...
| Daily CRM digest route | `src/app/api/crm/daily-digest/route.ts` | `tests/integration/api/crm-daily-digest.test.ts` | Lead digest for admin/service-role caller; blocked/high task inclusion; workspace-scoped task query; skip task reads when workspace scope missing; missing config; invalid query; safe lead/task read failures. | Covered with mocks/local route tests; latest progress log records 15-test focused gate pass. | Add contacts/opportunities once read routes exist; add stale-integration summary after thresholds are defined. |
...
| Margot voice-to-task ingress | `src/app/api/pi-ceo/margot-voice/task/route.ts` | `tests/integration/api/margot-voice-task.test.ts` | Bearer auth; rate limit; missing token/env; invalid JSON/packet; voice session insert failure; task insert failure; success no-store; summary truncation; defaults; approval-required blocked task behavior. | Covered in focused voice gate. | Need local schema provenance for `tasks` and `voice_command_sessions`; add digest reader linkage test when schema is formalized. |
...
| Activity/timeline | `supabase/migrations/20260510000004_nexus_agent_actions.sql`; `src/lib/empire/read-client-activity.ts`; `_record-action.ts` | `src/app/api/empire/clients/__tests__/record-action.test.ts` plus client route tests | Client create/update audit event behavior exists. | Partial; not yet a full CRM event taxonomy. | Decide `agent_actions` extension vs `crm_activity_timeline`; add event taxonomy tests for lead conversion/contact/opportunity/digest events. |
...
1. Opportunity read/digest integration so the daily CRM digest can surface open/won/blocked opportunities safely.
2. Activity/timeline event taxonomy: lead captured, qualified, converted, contact created, opportunity created, approval requested, task completed, integration stale.
3. CRM approvals lifecycle decision and tests: requested, approved, rejected, expired/cancelled, executed.
4. Command-center CRM UI read surface tests for leads, approvals, opportunities, and daily digest.
5. Integration stale-sync threshold tests for Linear/GitHub/Vercel/Supabase mirrors.
6. Local schema provenance for `tasks` and `voice_command_sessions`.
7. Wider regression including existing `src/app/api/empire/clients/**/__tests__` before any `nexus_clients` conversion work.
