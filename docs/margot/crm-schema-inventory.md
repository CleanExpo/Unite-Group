...
| Proposed `crm_contacts` | Canonical people/contact map for leads, clients, businesses, and stakeholders. | No migration/source found. Current contact data lives in `nexus_clients.contact_name/contact_email` and `crm_leads` fields. | None. | None. | None. | Gap / proposed. | Need table proposal, dedupe policy, client-mixing abort rules, and tests. |
| Proposed `crm_opportunities` | Qualified commercial opportunities with stage/value/probability/source. | No migration/source found. | None. | None. | None. | Gap / proposed. | Needed after lead qualification and before lead-to-client conversion. |
| Proposed `crm_approvals` or task subtype | Human decision/permission gates for production, billing, client-facing, identity, and cross-client decisions. | No dedicated migration/source found. Voice route currently uses `tasks.status='blocked'`, high priority, assignee `Phill approval`, approval tags/reason. | Voice task route creates approval-needed tasks. | Future command center/digest. | Margot voice task tests cover approval-required task behavior. | Partial via task convention, not dedicated table. | Need decide dedicated table vs task subtype; need approved/rejected outcome fields and audit events. |
| Proposed `crm_activity_timeline` | Unified timeline across leads, clients, contacts, opportunities, tasks, approvals, integrations, and voice. | No migration/source found. Current event/audit table is `agent_actions`. | Client audit helper writes `agent_actions`; voice route writes sessions/tasks. | Client activity helper reads `agent_actions`. | Client record-action tests; Margot voice tests. | Partial via `agent_actions`; broader timeline is a gap. | Need event taxonomy, object references, append-only behavior, and digest query contract. |
...
3. Contacts
   - Propose `crm_contacts` rather than continuing to overload lead/client email fields.
   - Include links to lead/client/business, role/title, consent/source, primary email/phone, dedupe keys, and privacy boundaries.
   - Add cross-client leakage abort rules.

4. Opportunities
   - Propose `crm_opportunities` with lead/client/contact links, stage, value, probability, expected close, source, owner, next action, and status.
...
6. Timeline/event model
   - Decide whether to extend `agent_actions` or introduce `crm_activity_timeline`.
