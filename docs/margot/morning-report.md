...
## 2026-06-23 11:59 AEST — PR #442 API auth redirect fix + E2E gate split

- **Completed safe lane:** Continued PR #442 (`fix/e2e-skip-without-backend`) in an isolated temp worktree. Fixed a real code-level auth/proxy failure before publishing any new lane: unauthenticated `/api/*` requests were being redirected to login, which Playwright followed to a 200 page.
- **PR:** https://github.com/CleanExpo/Unite-Group/pull/442 — `fix(e2e): skip backend-dependent specs when no E2E Supabase backend`.
- **What changed:** Added `apps/web/src/__tests__/proxy.test.ts` and updated `apps/web/src/proxy.ts` so unauthenticated non-public API paths return JSON 401 (`{ error: 'Unauthorised' }`) while browser/page paths still redirect to `/auth/login`.
- **TDD:** RED focused Vitest failed first with `expected 307 to be 401`; GREEN then passed after the minimal proxy guard change.
- **Verification:** Focused proxy Vitest passed (1 file / 1 test). On the original pre-advance PR head, focused Playwright auth passed after the proxy fix (3 passed); after rebasing onto current PR head `b48eae8d0`, the same auth spec skipped locally as expected because PR #442 now gates it behind the dedicated E2E backend when local backend config is absent. `pnpm run type-check && pnpm run lint && pnpm run test` passed (full Vitest: 387 files / 2303 tests); `pnpm run build` passed with safe CI placeholder env values only; `git diff --check` passed.
- **Remote gate read-back before push:** PR #442 head advanced during the run from `bb481cd91d07e163b0d1f90637af056e36311296` to `b48eae8d0217dfd5187854ac353a3a80d932a97b`; I reapplied the local proxy fix on top of the current branch before committing. Pre-fix checks had product-code/Vercel checks green and `apps/web — Playwright E2E` red. Failed-log signals were three API 401 expectations receiving 200 after redirect, plus the existing namespace config/provisioning class: blank `PLAYWRIGHT_TEST_EMAIL` / `PLAYWRIGHT_TEST_PASSWORD` and Supabase Auth `createUser failed: Database error creating new user`.
- **Gate packet:** `NAMESPACE` gate; `KEEP_GATED` for PR #442 until refreshed remote E2E is green or Phill/operator grants an explicit typed waiver. If only blank E2E secrets / Supabase test-user provisioning remain after push, authorised operator configuration/provisioning is required.
- **Safety:** No production DB write/migration, Vercel/GitHub secret mutation, billing/payment action, credential value read/print, client-facing send, cross-client merge, PR merge, or live provider mutation occurred. The dirty main checkout was not used for the code change.
- **Evidence paths:** `docs/margot/overnight-progress-log.md`, `docs/margot/morning-report.md`.
- **Next safe lane:** Commit/push PR #442 after branch/head read-back, then monitor refreshed checks; keep it gated if required E2E remains red on the known namespace class.
