...
Current as of `2026-05-31 12:51 AEST`: active branch `margot/tasks-voice-schema-proposal` is pushed at `92044bf` with no open GitHub PR yet. This checkpoint advances the next CRM command-spine slice by adding a sandbox-only migration proposal for `public.tasks` and `public.voice_command_sessions` at `docs/margot/migration-proposals/2026-05-31-tasks-voice-command-sessions-sandbox.sql`, plus regression coverage in `tests/unit/margot-tasks-voice-migration-proposal.test.ts`. The proposal is explicitly `PROPOSAL — not applied anywhere yet`, references `scripts/sandbox-wizard.sh`, uses additive/idempotent DDL, includes route-required task fields including `obsidian_synced_at`, enables RLS, and keeps policy creation/production promotion gated on sandbox validation plus fresh Board approval. TDD evidence: after adding `obsidian_synced_at` to the required-column test, the focused Jest suite failed as expected against the proposal; after patching both CREATE TABLE and ADD COLUMN sections, it passed. Verification passed: `npx jest tests/unit/margot-tasks-voice-migration-proposal.test.ts --runInBand` (1 suite / 17 tests), `npm run type-check`, `npm run security:routes-check` (0 unprotected mutating routes), `git diff --check`, spec review PASS, and quality review APPROVED with only non-blocking minor test-hardening suggestions. No production DB write, Supabase migration application, sandbox apply, Vercel env mutation/manual deploy, client-facing communication, billing/payment action, destructive git, cross-client merge, external account/vendor action, credential prompt, secret read, noninteractive auth attempt, or secret printing/storage occurred.

Current queue/status notes: GitHub auth was available for `CleanExpo` without token values printed; `gh pr list` returned no open PRs. This checkout also contains inherited/local dirty infrastructure and Supabase temp artifacts outside the CRM migration-proposal slice (`docs/runbooks/*`, `docs/backup-pipeline-assessment.md`, `docs/security/audit-2026-05-31.md`, `scripts/backup-healthcheck.sh`, `scripts/restoreassist-verify.sh`, `supabase/.temp/*`), so the safe publication path is to commit/push only the migration-proposal/test/Margot evidence paths or isolate unrelated DR work before opening a PR. Dimitri ITR tasks (UNI-2079, UNI-2080, UNI-2075) remain out-of-scope for this repo and belong to separate Vercel project `dimitri-itr-sandbox` / local `Unite-Hub`. CCW UNI-2053 draft copy exists at `docs/margot/ccw-carpet-cleaning-machines-category-copy.md` and remains blocked on Toby/Phill category approval; no public/client-facing action is approved.

Next safe slice: commit and push only the sandbox migration proposal/test/evidence paths on `margot/tasks-voice-schema-proposal`, open a PR if the worktree can be isolated cleanly, then apply the proposal to the sandbox with `scripts/sandbox-wizard.sh apply <file>` only after the sandbox prerequisites are available; do not promote to production without fresh Board approval.

Previous current as of `2026-05-31 11:36 AEST`: PR #213 (`https://github.com/CleanExpo/Unite-Group/pull/213`) is merged on `main` at `38f3b8f928f93e8e7a6acf84af3fb2612210d21e`. The merged test/mock/doc slice closes the next CRM coverage-matrix gap: shared Supabase reader mocks now record query-builder calls, and regressions pin Business 360/read-client-activity service-role query shape before broader client dashboard changes or any `nexus_clients` conversion path. `readClientActivity` must select only the expected `agent_actions` columns, filter `client_created`/`client_updated`, scope by exact `payload->>slug`, order newest first, and respect default/caller limits; `readBusiness360` must select only health snapshot fields, use a `snapshot_at` lower bound, order ascending, and cap reads at 10,000. Local verification before PR passed: focused slice gate 2 suites / 23 tests; expanded CRM/client gate 19 suites / 204 tests; bracketed `[slug]` PATCH suite 1 suite / 10 tests; `npm run type-check`; `npm run security:routes-check` with 0 unprotected mutating routes; `git diff --check`; spec review PASS; quality review APPROVED. PR checks passed, CodeRabbit skipped cleanly, both GitHub-triggered Vercel status contexts passed, and post-merge `main` CI run `26700100973` plus DESIGN.md lint run `26700100972` passed. Post-merge GitHub commit status for `38f3b8f` is success with `Vercel – unite-group` and `Vercel – unite-group-sandbox` both succeeded; these are status-check observations only, not manual Vercel deploy/env mutation. No production DB write, Supabase migration application, sandbox apply, Vercel env mutation/manual deploy, client-facing communication, billing/payment action, destructive git, cross-client merge, external account/vendor action, credential prompt, secret read, noninteractive auth attempt, or secret printing/storage occurred.
