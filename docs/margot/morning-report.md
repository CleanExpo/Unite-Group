...
Current as of `2026-05-31 08:29 AEST`: the next CRM coverage-matrix gap is complete as a local verification/documentation slice on branch `margot/crm-client-regression-matrix`. The focused CRM regression gate now includes existing Empire client route suites before future `nexus_clients` conversion work: 17 CRM/root-client suites passed with 181 tests, the bracketed `[slug]` PATCH validation suite passed separately with 10 tests, `npm run type-check` passed, `npm run security:routes-check` returned 0 unprotected mutating routes, and `git diff --check` passed after the documentation updates. `docs/margot/crm-test-coverage-matrix.md` now records the wider gate and marks gap #6 complete. No source/test behavior changed in this tick, and no production DB write, Supabase migration application, sandbox apply, Vercel env mutation/manual deploy, client-facing communication, billing/payment action, destructive git, cross-client merge, external account/vendor action, credential prompt, secret read, noninteractive auth attempt, or secret printing/storage occurred.

Current queue/status notes: no open PR was present at preflight, and GitHub auth was available for `CleanExpo` without token values printed. The prior PR #211 voice-task digest slice remains merged on `main` at `8993ea1b2e5cc2cff9699384dbfc06f5f905d5b7`; its post-merge `main` CI, DESIGN.md lint, and GitHub/Vercel status-check observations had already passed. The current evidence edits are on branch `margot/crm-client-regression-matrix`; no push/PR/merge/manual deploy has occurred yet. Dimitri ITR tasks (UNI-2079, UNI-2080, UNI-2075) remain out-of-scope for this repo and belong to separate Vercel project `dimitri-itr-sandbox` / local `Unite-Hub`. CCW UNI-2053 draft copy exists at `docs/margot/ccw-carpet-cleaning-machines-category-copy.md` and remains blocked on Toby/Phill category approval; no public/client-facing action is approved.

Next safe slice: add Business 360/read-client-activity regression coverage before broader client dashboard changes or any `nexus_clients` conversion path. Keep the same RED/GREEN and two-stage review discipline.
