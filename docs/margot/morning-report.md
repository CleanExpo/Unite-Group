...
Current as of `2026-05-31 07:13 AEST`: PR #211 (`feat: link voice tasks in CRM digest`) is open at `https://github.com/CleanExpo/Unite-Group/pull/211` on branch `margot/voice-task-digest-linkage`. The slice adds daily-digest reader linkage for Margot voice-created `tasks`: the route/digest mapper reads only minimal source-detection fields (`tags`, `obsidian_path`) in addition to existing task columns, marks rows matching the existing voice-ingress convention (`margot-voice` tag or `voice/<packet>` path) as `source: 'margot_voice'`, and renders operator-facing digest copy as `Voice task ...` while preserving existing approval/priority behavior. Initial PR CI found one stale full-suite unit assertion in `tests/unit/lib/crm/read-daily-digest.test.ts`; I patched it to expect the same minimized task column set and reran review/verification. Local verification now passes: focused full-digest gate 4 suites / 39 tests, full `npx jest --runInBand` 142 passed / 1 skipped / 1102 passed tests, `npm run type-check`, and `npm run security:routes-check` (0 unprotected mutating routes). Spec review passed and quality/security review approved for both the implementation and the CI micro-fix.

Current queue/status notes: PR #211 is open; GitHub auth is available for `CleanExpo` without token values printed. Latest observed PR checks for head `8a27e7c` have all required Review Board/TypeScript/lint/schema/security/Vercel contexts passing except `Unit + Integration Tests`, which failed before the micro-fix was pushed. Vercel status checks `Vercel – unite-group` and `Vercel – unite-group-sandbox` both succeeded as GitHub status observations only; no manual Vercel deploy/env mutation occurred. Dimitri ITR tasks (UNI-2079, UNI-2080, UNI-2075) remain out-of-scope for this repo and belong to separate Vercel project `dimitri-itr-sandbox` / local `Unite-Hub`. CCW UNI-2053 draft copy exists at `docs/margot/ccw-carpet-cleaning-machines-category-copy.md` and remains blocked on Toby/Phill category approval; no public/client-facing action is approved.

Next safe slice: commit and push the CI micro-fix for PR #211, then monitor the rerun status checks. After this branch is merged or parked, the next small CRM gap is a wider regression including existing `src/app/api/empire/clients/**/__tests__` before touching `nexus_clients` conversion.
