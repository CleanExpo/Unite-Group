...
Current as of `2026-05-31 06:56 AEST`: active local branch `margot/voice-task-digest-linkage` adds the next CRM coverage-matrix slice: daily-digest reader linkage for Margot voice-created `tasks`. The route/digest mapper now reads only minimal task source-detection fields (`tags`, `obsidian_path`) in addition to existing task columns, marks rows matching the existing voice-ingress convention (`margot-voice` tag or `voice/<packet>` path) as `source: 'margot_voice'`, and renders operator-facing digest copy as `Voice task ...` while preserving existing approval/priority behavior. RED evidence: `npx jest tests/integration/api/crm-daily-digest.test.ts --runInBand` failed when the digest still rendered `Task ...`; GREEN/focused verification passed after implementation: 3 suites / 31 tests across daily digest route/helper and Margot voice task ingress, `npm run type-check`, and `npm run security:routes-check` (0 unprotected mutating routes). Spec review passed and quality/security review approved. This is local-only until committed/pushed/PR-created; no production DB write, migration application, Vercel env mutation/manual deploy, client-facing send, public publishing, external vendor/account action, or secret printing/storage occurred.

Current queue/status notes: no open PRs for Unite-Group; GitHub auth is available for `CleanExpo` without token values printed. Dimitri ITR tasks (UNI-2079, UNI-2080, UNI-2075) remain out-of-scope for this repo and belong to separate Vercel project `dimitri-itr-sandbox` / local `Unite-Hub`. CCW UNI-2053 draft copy exists at `docs/margot/ccw-carpet-cleaning-machines-category-copy.md` and remains blocked on Toby/Phill category approval; no public/client-facing action is approved.

Next safe slice: run final diff hygiene, commit/push/open PR for `margot/voice-task-digest-linkage` if GitHub transport remains available, then monitor CI/Vercel status checks. After this branch is merged or parked, the next small CRM gap is a wider regression including existing `src/app/api/empire/clients/**/__tests__` before touching `nexus_clients` conversion.
