...
export function HermesControlPanel({ initialPayload, dailyDigestInitial }: HermesControlPanelProps = {}) {
  ...
  const capabilityDecisions = payload?.capabilityDecisions ?? [];
  const badgeMode =
    sourceState === 'live'
      ? 'live'
      : sourceState === 'loading'
  ...
  return (
    ...
    >
      ...
      {capabilityDecisions.length > 0 && (
        <CapabilityDecisionQueue items={capabilityDecisions} />
      )}
...
function CapabilityDecisionQueue({ items }: { items: CapabilityDecision[] }) {
  return (
    <section
      className="grid gap-4 px-6 py-5 lg:grid-cols-[16rem_1fr]"
      style={{
        background: 'var(--cc-bg-soft)',
        borderBottom: '1px solid var(--cc-grid)',
      }}
      aria-label="Compound development approval queue"
    >
      <div className="flex flex-col gap-2">
        <span
          className="font-mono text-[10px] uppercase tracking-[0.22em]"
      ...
      <div className="grid gap-px overflow-hidden border" style={{ borderColor: 'var(--cc-grid)', background: 'var(--cc-grid)' }}>
        {items.map((item) => (
          <article
            key={item.id}
            className="grid gap-3 px-4 py-3 md:grid-cols-[1fr_11rem]"
          ...
          >
            <div className="flex min-w-0 flex-col gap-2">
              <span
                ...
                style={{ color: 'var(--cc-ink-hush)' }}
              >
                {item.priority ?? 'normal'} / {item.status}
              </span>
              <h4
                className="text-sm font-semibold leading-snug"
                style={{ color: 'var(--cc-ink)' }}
            ...
            <div className="flex flex-col gap-2 font-mono text-[11px] leading-relaxed">
              <MetaLine label="owner" value={item.assigneeName ?? 'Unassigned'} />
              <MetaLine label="task" value={item.id} />
              <MetaLine label="updated" value={formatShortDate(item.lastUpdated)} />
              {item.tags.length > 0 && (
                <span
