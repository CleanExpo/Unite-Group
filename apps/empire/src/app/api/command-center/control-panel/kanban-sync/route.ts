...
function syncDue(task: TaskRow) {
  if (['done', 'closed', 'complete', 'completed'].includes(task.status.trim().toLowerCase())) {
    return false;
  }
...
function kanbanPriority(priority: string | null) {
...
function toSyncPacket(task: TaskRow) {
  ...
  return {
    ...
    kanban: {
      title: safeTitle,
      body: bodyFor(task),
      status: task.status.trim().toLowerCase() === 'blocked' ? 'blocked' : 'triage',
      priority: kanbanPriority(task.priority),
    },
  };
}
