...
function uniqueTags(tags: string[], requiredTags: string[]) {
  return Array.from(new Set([...tags.map(normalizeTag), ...requiredTags.map(normalizeTag)]))
    .filter(Boolean)
    .slice(0, 20);
}
...
function parseProposal(value: unknown): CapabilityIntakeProposal | null {
  ...
  if (!normalizedTags.includes('capability-scout') || !normalizedTags.includes('approval-required')) return null;
  ...
  return {
    ...
    tags: uniqueTags(tags, [capabilityType, hermesLane, 'hermes-intake']),
