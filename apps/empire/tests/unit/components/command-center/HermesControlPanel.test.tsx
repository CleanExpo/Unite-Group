...
describe('HermesControlPanel', () => {
  ...
  it('renders compound-loop capability decisions from the CRM payload', () => {
    const html = renderToStaticMarkup(
      <HermesControlPanel
        initialPayload={{
          source: 'crm:tasks',
          taskCount: 1,
          generatedAt: '2026-06-16T04:15:00.000Z',
          summary: {
            green: 0,
            yellow: 0,
            red: 1,
          ...
          capabilityDecisions: [
            {
              ...
              status: 'blocked',
              priority: 'urgent',
              assigneeName: 'Phill approval',
              obsidianPath: 'Outcomes/idea-expansion/restoreassist-agent-runtime.md',
              lastUpdated: '2026-06-16T04:00:00.000Z',
              tags: [
                'capability-scout',
                'approval-required',
                'idea-expansion',
    ...
    expect(html).toContain('aria-label="Compound development approval queue"');
    expect(html).toContain('Capability decisions');
    expect(html).toContain('1 awaiting approval');
    expect(html).toContain('Review capability: compound agent runtime for RestoreAssist');
    expect(html).toContain('Outcomes/idea-expansion/restoreassist-agent-runtime.md');
    expect(html).toContain('Phill approval');
    expect(html).toContain('capability-scout / approval-required / idea-expansion');
  });
