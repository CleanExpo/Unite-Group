...
describe('GET /api/command-center/control-panel', () => {
  ...
  it('surfaces compound-loop capability approvals as a dedicated decision queue', async () => {
    const limit = jest.fn().mockResolvedValue({
      data: [
        {
          id: 'task-capability-1',
          title: 'Review capability: compound agent runtime for RestoreAssist',
          status: 'blocked',
          priority: 'urgent',
          tags: [
            'capability-scout',
          ...
          assignee_name: 'Phill approval',
          obsidian_path: 'Outcomes/idea-expansion/restoreassist-agent-runtime.md',
          updated_at: '2026-06-16T04:00:00.000Z',
          created_at: '2026-06-16T03:55:00.000Z',
        },
        {
          id: 'task-capability-2',
          title: 'Review capability: low priority content model',
          status: 'blocked',
          priority: 'low',
          tags: ['capability-scout', 'approval-required', 'idea-expansion'],
          assignee_name: 'Operator',
          obsidian_path: 'Outcomes/idea-expansion/low-priority.md',
          updated_at: '2026-06-16T04:05:00.000Z',
          created_at: '2026-06-16T04:00:00.000Z',
        },
        {
          id: 'task-normal-idea',
          title: 'Research expansion: normal monitoring pass',
          status: 'running',
          priority: 'normal',
          tags: ['idea-expansion'],
          assignee_name: 'Operator',
          obsidian_path: 'Outcomes/idea-expansion/normal.md',
          updated_at: '2026-06-16T04:10:00.000Z',
    ...
    expect(res.status).toBe(200);
    expect(body.source).toBe('crm:tasks');
    expect(body.summary.approvalRequired).toBe(2);
    expect(body.capabilityDecisions).toHaveLength(2);
    expect(body.capabilityDecisions[0]).toMatchObject({
      id: 'task-capability-1',
      title: 'Review capability: compound agent runtime for RestoreAssist',
      priority: 'urgent',
    });
    expect(body.capabilityDecisions[1]).toMatchObject({
      id: 'task-capability-2',
      title: 'Review capability: low priority content model',
      priority: 'low',
    });
