...
This crm-contacts-opportunities-model doc is now bound to the local, mocked AI-RET-001 retrieval-evaluation harness (`src/lib/margot/retrieval-evaluation.ts`, `scripts/margot-retrieval-evaluation-report.ts`, `docs/margot/evidence/AI_RET_001_LOCAL_RETRIEVAL_REPORT.md`) via the 91st answer-shape fixture `AI-RET-001-ANSWER-CONTACTS-OPPORTUNITIES-MODEL-SELF-BOUNDARY` (bound to `AI-RET-001-CONTACTS-OPPORTUNITIES-MODEL`, no source-citation union member added). The 91st is a *self-boundary*: it pins the literal evidence identifiers in this doc (sandbox only draft, no production apply, forecast only, stripe remains billing truth, strong identity gates, operator approval, cross client leakage abort, crm_contacts and crm_opportunities draft migration in migration proposals directory, sandbox wizard only promotion path, use existing assets first) so a future answer about the crm-contacts-opportunities-model self-boundary is forced to cite this doc, the crm-operating-model, the lead-to-client-conversion-plan, and the senior-pm-operating-model, and is bound to the canonical sandbox-only-draft contract.

The 91st fixture enforces:

- 10 required answer phrases: `crm contacts opportunities model self boundary lane`, `9th contacts opportunities safety boundary content citation class`, `sandbox only draft crm_contacts and crm_opportunities migration`, `no production apply until sandbox wizard authority auth gate and board approval`, `forecast only opportunity value probability and expected close`, `stripe remains billing truth crm mirror must not write billing fields`, `strong identity gates and operator approval for any contact or opportunity action`, `cross client leakage abort on ambiguous identity or weak dedupe proof`, `crm_contacts and crm_opportunities draft migration lives in migration proposals directory`, `use existing assets first`.
- 4 required citation sources: `docs/margot/crm-contacts-opportunities-model.md`, `docs/margot/crm-operating-model.md`, `docs/margot/lead-to-client-conversion-plan.md`, `docs/margot/SENIOR-PROJECT-MANAGER-OPERATING-MODEL.md`.
- 10 prohibited overclaim phrases: `crm contacts opportunities model crm_contacts production apply`, `crm contacts opportunities model crm_opportunities production apply`, `crm contacts opportunities model contact auto created`, `crm contacts opportunities model opportunity auto created`, `crm contacts opportunities model cross client merge applied`, `crm contacts opportunities model billing field written`, `crm contacts opportunities model sandbox wizard apply run without authority`, `crm contacts opportunities model nango connector platform onboarded`, `crm contacts opportunities model github push executed`, `crm contacts opportunities model vercel deploy executed`.

The 91st fixture is the 23rd self-boundary for a committed source doc (37th = disaster-recovery-assessment-boundary, 64th = dr-swarm-execution-report, 66th = dr-validation-gap-analysis, 67th = sandbox-wizard-credential-boundary-review, 72nd = voice-test-gap-analysis, 73rd = access-and-data-requirements, 74th = retrieval-rules, 75th = high-level-crm-25-step-forecast, 76th = personal-intelligence-candidate-register, 77th = command-center, 78th = margot-orchestrator, 79th = second-brain-carry-forward, 80th = senior-project-manager-operating-model, 81st = connected-teams-operating-rules, 82nd = mac-mini-recovery-status, 83rd = crm-operating-model, 84th = crm-test-coverage-matrix, 85th = marketing-strategy-operating-model, 86th = client-second-brain-model, 87th = project-portfolio-index, 88th = voice-task-schema-provenance, 89th = crm-schema-inventory, 90th = crm-approval-persistence-plan, 91st = crm-contacts-opportunities-model). The 91st is deliberately disjoint from the 9th content-citation `AI-RET-001-ANSWER-CONTACTS-OPPORTUNITIES-SAFETY-BOUNDARY` fixture (which guards the operator-evidence contacts-opportunities surface map); the 9th is bound to `AI-RET-001-COMMAND-CENTER-CITATION`, the 91st is bound to `AI-RET-001-CONTACTS-OPPORTUNITIES-MODEL`. The two cover different coverage vectors (operator-evidence surface map vs self-evidence identifier set).

The local AI-RET-001 harness enforces that crm-contacts-opportunities-model answers remain source-labeled, local-only, and bound to the sandbox-only-draft contract. No crm_contacts production apply, no crm_opportunities production apply, no contact auto created, no opportunity auto created, no cross client merge applied, no billing field written, no sandbox wizard apply run without authority, no nango connector platform onboarded, no github push executed, and no vercel deploy executed may be asserted as live state.

## Senior PM verification checkpoint (2026-06-12 19:30:00 AEST)

Doc-drift guard: the 10 required phrases (crm contacts opportunities model self boundary lane, 9th contacts opportunities safety boundary content citation class, sandbox only draft crm_contacts and crm_opportunities migration, no production apply until sandbox wizard authority auth gate and board approval, forecast only opportunity value probability and expected close, stripe remains billing truth crm mirror must not write billing fields, strong identity gates and operator approval for any contact or opportunity action, cross client leakage abort on ambiguous identity or weak dedupe proof, crm_contacts and crm_opportunities draft migration lives in migration proposals directory, and use existing assets first) and 4 required citations (crm-contacts-opportunities-model.md, crm-operating-model.md, lead-to-client-conversion-plan.md, SENIOR-PROJECT-MANAGER-OPERATING-MODEL.md) are present in the assertion section above. The 10 prohibited phrases are documented only here for completeness and do not appear in the assertion section; their presence here satisfies the answer-shape contract: crm contacts opportunities model crm_contacts production apply, crm contacts opportunities model crm_opportunities production apply, crm contacts opportunities model contact auto created, crm contacts opportunities model opportunity auto created, crm contacts opportunities model cross client merge applied, crm contacts opportunities model billing field written, crm contacts opportunities model sandbox wizard apply run without authority, crm contacts opportunities model nango connector platform onboarded, crm contacts opportunities model github push executed, crm contacts opportunities model vercel deploy executed.
