...
- The `## Senior PM verification checkpoint` block below records the current state, the missing authority/auth gates, the current health evidence, the Mac Mini state, and the smallest next action for this control surface. Do not validate or promote schema for `crm_leads`, `crm_contacts`, `crm_opportunities`, or the proposed `crm_approvals` from this inventory task; any schema change is validated on a Supabase database branch (never prod) and promoted to prod only via a merged, approved branch.
...
3. Contacts
   ...
   - Next: validate on a Supabase database branch (never prod) before any promotion, then add server-route/mocked tests for contact creation/linking.
...
4. Opportunities
   ...
   - Next: validate on a Supabase database branch (never prod) before any promotion, then add server-route/mocked tests for opportunity drafts.
...
Database-branch rule:

- Any schema change must be validated on a Supabase database branch (an ephemeral per-branch DB that replays `apps/web/supabase/migrations/`) before production promotion. Never validate against prod.
- Promote to prod (`lksfwktwtmyznckodsau`) only by merging an approved branch; never apply migrations to production directly or autonomously from this inventory task.
...
- `crm_contacts` is draft only, branch-first validation (Supabase database branch, never prod), and not yet promoted to a target environment.
- `crm_opportunities` is draft only, branch-first validation (Supabase database branch, never prod), and not yet promoted to a target environment.
...
- All schema changes follow the branch-first DB rule: validate on a Supabase database branch (never prod) before any production promotion, and promote to prod only via a merged, approved branch.
...
- Smallest next action: re-run the combined local gate and the AI-RET-001 report runner to confirm `source=8/8; answerShape=12/12; readback=pass`, regenerate the evidence report, and rotate to another bounded Senior PM lane (e.g. add another mocked AI-RET-001 answer-shape fixture for another gated boundary, refresh another control surface, or close a remaining voice-test gap from `docs/margot/voice-test-gap-analysis.md`). Do not validate any schema against prod or promote any migration to prod until the specific authority/auth gate changes; schema changes are validated on a Supabase database branch (never prod) and promoted to prod only via a merged, approved branch.
