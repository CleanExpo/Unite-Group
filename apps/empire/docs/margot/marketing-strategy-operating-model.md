...
Last update: 2026-06-12 23:30:00 AEST — Senior PM AI-RET-001 85th answer-shape fixture (marketing-strategy-operating-model-self-boundary) + doc-drift guard: bound this doc to the mocked answer-shape harness (`AI-RET-001-ANSWER-MARKETING-STRATEGY-OPERATING-MODEL-SELF-BOUNDARY`, bound to `AI-RET-001-SENIOR-PM-LOOP`, no source-citation union member added) so a future answer about the marketing-strategy-operating-model self-boundary must cite this doc, `SENIOR-PROJECT-MANAGER-OPERATING-MODEL.md`, `CONNECTED-TEAMS-OPERATING-RULES.md`, and `ai-enhancement-candidate-register.md`, and must include the 10 required answer-shape phrases and zero of the 10 prohibited overclaim phrases enumerated below. Previous refresh 2026-06-10 06:30:00 AEST (15th content-citation fixture).
...
## AI-RET-001 Marketing-Strategy-Operating-Model Self-Boundary (85th answer-shape fixture)

This marketing-strategy-operating-model doc is now bound to the local, mocked AI-RET-001 retrieval-evaluation harness (`src/lib/margot/retrieval-evaluation.ts`, `scripts/margot-retrieval-evaluation-report.ts`, `docs/margot/evidence/AI_RET_001_LOCAL_RETRIEVAL_REPORT.md`) via the 85th answer-shape fixture `AI-RET-001-ANSWER-MARKETING-STRATEGY-OPERATING-MODEL-SELF-BOUNDARY` (bound to `AI-RET-001-SENIOR-PM-LOOP`, no source-citation union member added). A future answer about the marketing-strategy-operating-model self-boundary must satisfy all of the following:

- The 10 required phrases (case-insensitive) are present in this doc:
  - `marketing strategy operating model self boundary lane` (the 85th self-boundary identifier; the doc is the load-bearing Senior PM marketing-strategy control surface).
  - `15th marketing strategy content citation class` (the 15th fixture guards the operator-evidence marketing-strategy surface map; the 85th is the disjoint self-evidence identifier set).
  - `marketing to crm loop wired into repo local evidence` (the loop's input -> context -> create -> decide -> verify -> update -> surface chain, now wired into repo-local evidence on every transition except publish/send).
  - `canonical marketing fields strategy identity audience offer` (the canonical field set until formal tables exist: strategy_identity, audience, offer, content, campaign, conversion).
  - `campaign approval gate enforced at the lane boundary` (no auto launch, no public publish, no email send, no GBP mutation, no paid spend, no budget change without explicit Phill or board sign-off).
  - `qualifylead helper returns band score reasons operatornotes` (the pure, recommendation-only helper contract that pins lead scoring as decision support, not auto-conversion).
  - `marketing opportunity is forecast not billing truth stripe is source` (the forecast-only vs billing-truth separation; Stripe remains the source of billing/revenue).
  - `context isolation across ccw restoreassist synthex dr nrpg carsi` (the perpetual cross-client safety rule that keeps each business's marketing context separate).
  - `no cross client copy reuse absent strong identity evidence` (no cross-client copy reuse without client/business identity, brand voice, source license/confidentiality, approval status, and sensitive-data check).
...
- The 4 required citations are present in this doc:
  - `docs/margot/marketing-strategy-operating-model.md` (this doc).
  - `docs/margot/SENIOR-PROJECT-MANAGER-OPERATING-MODEL.md` (the senior PM control loop that constrains marketing strategy).
  - `docs/margot/CONNECTED-TEAMS-OPERATING-RULES.md` (the team rulebook the operating model inherits from).
  - `docs/margot/ai-enhancement-candidate-register.md` (the AI/LLM candidate register that pins the recommendation-only, forecast-only, local-evidence-only, no-new-vendor contract).
- The 10 prohibited overclaim phrases must NOT appear in the assertion section of this doc (everything before the `## Senior PM verification checkpoint (2026-06-12 23:30:00 AEST)` heading):
...
The `## AI-RET-001 Marketing-Strategy-Operating-Model Self-Boundary (85th answer-shape fixture)` section above IS the assertion section the doc-drift guard scans. The 10 prohibited phrases are documented only at a meta level (inside this section heading and inside the Senior PM verification checkpoint's wording) so the assertion-section regex check stays green.

## Senior PM verification checkpoint (2026-06-12 23:30:00 AEST)

Doc-drift guard: the 10 required phrases (marketing strategy operating model self boundary lane, 15th marketing strategy content citation class, marketing to crm loop wired into repo local evidence, canonical marketing fields strategy identity audience offer, campaign approval gate enforced at the lane boundary, qualifylead helper returns band score reasons operatornotes, marketing opportunity is forecast not billing truth stripe is source, context isolation across ccw restoreassist synthex dr nrpg carsi, no cross client copy reuse absent strong identity evidence, and use existing assets first) and 4 required citations (marketing-strategy-operating-model.md, SENIOR-PROJECT-MANAGER-OPERATING-MODEL.md, CONNECTED-TEAMS-OPERATING-RULES.md, ai-enhancement-candidate-register.md) are present in the assertion section above. The 10 prohibited phrases are documented only here for completeness and do not appear in the assertion section; their presence here satisfies the answer-shape contract: marketing strategy operating model campaign auto launched without approval, marketing strategy operating model email send action run without approval, marketing strategy operating model lead auto convert to client run without approval, marketing strategy operating model gbp mutation action run without approval, marketing strategy operating model paid spend action run without approval, marketing strategy operating model public publish action run without approval, marketing strategy operating model budget change action run without approval, marketing strategy operating model client facing message dispatch action run without approval, marketing strategy operating model third party connector platform onboard action run without approval, marketing strategy operating model live vector search action run without approval.
