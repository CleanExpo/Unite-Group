...
- No direct or autonomous production DB writes/migrations. Schema work is branch-first: write the migration in `apps/web/supabase/migrations/`, validate it on a Supabase database branch (ephemeral per-branch DB; never validate against prod), and promote to prod (`lksfwktwtmyznckodsau`) only by merging an approved branch with Phill's explicit typed approval. Static review packets and evidence reports are still allowed and encouraged, but no DB-writing step may run against prod directly.
...
- Smallest useful next action: keep this access policy aligned with the current CRM/voice/digest/approval/redaction lane and rotate to another bounded Senior PM lane (e.g. author the proposed `docs/margot/access-register.md`, `docs/margot/decision-rights-matrix.md`, or `docs/margot/identity-resolution-policy.md` from the Immediate Next 10 Actions, or close a voice-test gap from `docs/margot/voice-test-gap-analysis.md`). Do not write to the production DB directly or autonomously: any schema change is branch-first — write the migration in `apps/web/supabase/migrations/`, validate it on a Supabase database branch (never validate against prod), and promote to prod only by merging an approved branch with Phill's explicit typed approval. Do not use Nango or any other third-party connector platform. Do not perform a live vector search, embeddings backfill, or live AI call against production.
