...
4. ~~Add integration stale-sync threshold tests for Linear/GitHub/Vercel/Supabase mirrors.~~ **Completed at 2026-05-23 lane and extended through 2026-06-14**: `src/lib/runtime/stale-sync-check.ts` now handles `last_error` separately, clamps malformed `next_sync_due_at` and `last_sync_completed_at` to `never_synced` / `minutes_overdue=0`, falls back from malformed `next_sync_due_at` to `last_sync_completed_at + cadenceMs` for ok rows, and `tests/unit/lib/runtime/stale-sync-check.test.ts` (12 tests) covers happy-path, `last_error` precedence, missing `next_sync_due_at`, malformed timestamps, never-synced, healthy, and unknown-integration branches.
