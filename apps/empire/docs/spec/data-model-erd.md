...
> **Companion artifact to [`/spec.md`](../../spec.md) §6 (Domain & Data Architecture).** Full entity model, ERD, dedupe-key strategy, privacy scopes, RLS posture, and the branch-first migration plan for the V1 tables.
>
...
>
> **Migration law (locked):** every schema change is written in `apps/web/supabase/migrations/`, validated on a **Supabase database branch** (an ephemeral per-branch DB that replays the migrations — never validate against prod), and promoted to prod ref `lksfwktwtmyznckodsau` **only** by merging an approved branch with Phill's typed approval — never applied to prod directly or autonomously. No `psql`, `supabase db push`, or MCP `apply_migration` against prod ref `lksfwktwtmyznckodsau`. (The old mirror sandbox `xgqwfwqumliuguzhshwv` was deleted 2026-06-15 and will not be replaced.)
>
...
| `crm_contacts` | DRAFTED, not yet validated on a branch or promoted to prod | Canonical people record | `20260523103000_crm_contacts_opportunities.sql` `[VERIFIED file]` |
| `crm_opportunities` | DRAFTED, not yet validated on a branch or promoted to prod | Pipeline / forecast (forecast-only, NOT billing truth) | `20260523103000_crm_contacts_opportunities.sql` `[VERIFIED file]` |
...
**V1 hardening (NEW additive migration, branch-first):**
...
**RLS acceptance (verify on the database branch post-apply):**
...
- **`crm_approvals` table is V2** — built branch-first only when structured approval history/query needs are proven (states requested/approved/rejected/expired/cancelled/executed; links requester/approver/reason/scope/risk/related-object/audit-event). `[VERIFIED plan]`
...
## 7. Branch-first migration plan (V1 tables → prod)

Every migration is written in `apps/web/supabase/migrations/` and validated on a **Supabase database branch** (an ephemeral per-branch DB that replays the migrations); prod ref `lksfwktwtmyznckodsau` is reached **only** by merging an approved branch — never applied directly or autonomously. Sourced from `apps/empire/CLAUDE.md` and `CLAUDE.md`. `[VERIFIED]`

```mermaid
flowchart TD
    A["M0: Prereqs — op signin; SUPABASE_ACCESS_TOKEN; UNITE_GROUP_DB_PASSWORD"] --> B["M1: open a Supabase database branch off the migration PR (Supabase GitHub integration or create_branch) — replays supabase/migrations/"]
    B --> C["M2: confirm 20260523100000_crm_leads.sql is present on the branch (if not already in prod schema)"]
    C --> D["M3: confirm 20260523103000_crm_contacts_opportunities.sql on the branch"]
    D --> E["M4: add NEW hardening migration: dedupe UNIQUE index + phone/name_company keys + updated_at trigger + agent_actions append-only trigger + agent_actions SELECT founder-only + value_currency NOT NULL DEFAULT 'AUD' + CHECK + R6 nexus_clients ref column + payload->>slug GIN index + crm_convert_lead_to_contact() RPC + (deferred) authenticated SELECT policy"]
    E --> F["M5: review branch schema vs prod (diff the branch result)"]
    F --> G["M6: security advisor on the branch (get_advisors)"]
    G --> H{"Clean diff + 0 new security findings?"}
    H -->|no| D
    H -->|yes| I["M7: Board/Phill approval recorded"]
    I --> J["M8: promote to prod by MERGING the approved branch (never apply to prod directly)"]
    J --> K["M9: post-merge verify — generate_typescript_types; smoke read via routes; pg_policies/rowsecurity assertions"]
```

**Ordering constraint (verified):** `crm_contacts` must exist before `crm_opportunities` (FK `linked_contact_id → crm_contacts(id)`), and both must exist after `crm_leads` and `nexus_clients`/`businesses`. The combined migration already orders contacts (`…103000.sql:6`) before opportunities (`:80`) and applies as one transaction. `[VERIFIED]`

**Idempotency note:** all CRM migrations use `create table if not exists` + `do $$ … if not exists … create policy …` guards, so re-running them on an already-seeded database branch is safe. The new hardening migration must follow the same discipline. `[VERIFIED]`

---

## 8. "Data layer is done when" (V1 acceptance)

0. **(Pre-M1.1, mandatory) `types/supabase.ts` regenerated from prod ref `lksfwktwtmyznckodsau` and committed as the §17 evidence baseline; M1.1 verifies `crm_leads`, `nexus_clients`, `businesses` exist in prod BEFORE the contacts/opportunities branch is merged.**
1. `crm_contacts` and `crm_opportunities` exist in prod, reached **only** by merging an approved Supabase database branch (never applied to prod directly), with a clean branch-vs-prod diff and zero new security-advisor findings captured as evidence.
