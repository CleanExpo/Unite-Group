...
Currently configured project: `Unite-Group` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/Unite-Group/SETUP.md` to fill in
   `data/Unite-Group/INFO.md`.
...
`--project-id Unite-Group` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/Unite-Group/`.
