...
function packet(
  overrides: Partial<WorkPacket> & { id: string; status: PacketStatus },
): WorkPacket {
  return {
...
const samplePackets: WorkPacket[] = [
  packet({
    id: "wp-draft-1",
    status: "draft",
    outcome: "Draft a launch brief",
  }),
  packet({
    id: "wp-routed-1",
    status: "routed",
    outcome: "Route the coding job",
    lane: "coding",
    nextActionOwner: "senior_agent",
  }),
  packet({
    id: "wp-running-1",
    status: "running",
    outcome: "Run the render pipeline",
  }),
  packet({
    id: "wp-blocked-1",
    status: "blocked",
    outcome: "Blocked on credentials",
  }),
  packet({
    id: "wp-approval-1",
...
describe("WorkPacketLane", () => {
  ...
  it("renders a column per status with packets grouped into them", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      status: 200,
      json: async () => ({ packets: samplePackets }),
    });
  ...
  it("shows an Approve action only on an awaiting_approval card and posts the approve transition", async () => {
    ...
    const body = JSON.parse(
      (transitionCall?.[1] as RequestInit).body as string,
    );
