...
export function ReceivablesTab() {
  ...
  const fetchData = useCallback(async (biz: string) => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/xero/invoices?business=${biz}&type=ACCREC`)
      if (res.status === 503) {
        setError('Xero integration not configured for this business.')
        setInvoices([])
        return
      }
      if (!res.ok) throw new Error(`Failed to fetch receivables: ${res.status}`)
  ...
  // Total outstanding = sum of AmountDue (Xero returns dollars, convert to cents)
      ...
      {!loading && !error && (
        ...
        >
          ...
          <span className="text-[20px] font-semibold tabular-nums" style={{ color: 'var(--color-text-primary)' }}>
            {formatAUD(totalOutstandingCents)}
          </span>
        </motion.div>
      )}
          ...
          <table className="w-full text-[12px]">
            <thead>
              <tr className="border-b border-[var(--color-border)]" style={{ backgroundColor: 'var(--surface-card)' }}>
                <th className="text-left px-3 py-2.5 font-medium" style={{ color: 'var(--color-text-disabled)' }}>Contact</th>
                <th className="text-left px-3 py-2.5 font-medium" style={{ color: 'var(--color-text-disabled)' }}>Invoice #</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5">
                        <InvoiceStatusBadge status={inv.Status} dueDate={dueDate} />
                      </td>
                    </motion.tr>
                  )
                })
