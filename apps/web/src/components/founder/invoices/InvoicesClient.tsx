...
function StatusBadge({ status }: { status: XeroInvoice['Status'] }) {
  const map: Record<XeroInvoice['Status'], { bg: string; label: string }> = {
    ...
    AUTHORISED: { bg: 'rgba(22, 163, 74,0.15)',   label: 'Authorised' },
          ...
          <div className="flex items-center gap-1">
            {STATUS_OPTIONS.map((s) => (
              <button
                ...
                style={{
                  background: filterStatus === s ? 'rgba(22, 163, 74,0.15)' : 'rgba(255,255,255,0.05)',
                  color: filterStatus === s ? '#16a34a' : 'rgba(255,255,255,0.4)',
                }}
              >
