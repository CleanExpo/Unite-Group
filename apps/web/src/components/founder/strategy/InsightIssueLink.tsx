...
export function InsightIssueLink({ insightId, onLinked }: InsightIssueLinkProps) {
  ...
  async function submit() {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/strategy/insights/${insightId}/create-issue`, {
        method: 'POST',
  ...
  if (link) {
    return (
      <div className="flex items-center gap-2 mt-3 text-[11px]" style={{ color: 'var(--color-text-muted)' }}>
        ...
        {link.linear_issue_url ? (
          ...
          <span className="font-medium" style={{ color: 'var(--color-text-primary)' }}>{link.linear_issue_id}</span>
        )}
        {link.autonomous && (
          <span className="text-[10px] px-1.5 py-0.5 rounded-sm" style={{ color: '#15803d', background: 'rgba(22, 163, 74,0.1)' }}>
            autonomous
          </span>
        )}
      </div>
  ...
  return (
    <div className="mt-3">
      {!open ? (
        ...
        <div className="flex flex-col gap-2 rounded-sm border p-3" style={{ borderColor: 'var(--color-border)' }}>
          ...
          <textarea
            ...
            rows={3}
            placeholder="What done looks like — the issue can't be claimed autonomously without this."
          ...
          <label className="flex items-center gap-2 text-[11px]" style={{ color: 'var(--color-text-secondary)' }}>
            ...
            Make claim-eligible for autonomous work (adds {`'mesh:auto'`} label)
          </label>
          {error && (
