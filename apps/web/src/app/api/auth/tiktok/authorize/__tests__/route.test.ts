...
function req(business?: string) {
  const url = business
    ? `https://app.test/api/auth/tiktok/authorize?business=${business}`
    : 'https://app.test/api/auth/tiktok/authorize'
  return new Request(url)
}

describe('GET /api/auth/tiktok/authorize', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(requireOAuthEnv).mockReturnValue({ ok: true, missing: [], response: null })
    vi.stubEnv('TIKTOK_CLIENT_KEY', 'tt-client-key')
  ...
  it('returns 503 when TikTok env not configured', async () => {
    vi.mocked(getUser).mockResolvedValue({ id: 'user-1' } as any)
  ...
  it('returns 307 redirect to TikTok when configured', async () => {
    vi.mocked(getUser).mockResolvedValue({ id: 'user-1' } as any)
    const res = await GET(req('biz'))
