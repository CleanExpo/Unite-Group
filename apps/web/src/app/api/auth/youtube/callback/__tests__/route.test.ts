...
function req(params: Record<string, string> = {}) {
  const url = new URL('https://app.test/api/auth/youtube/callback')
...
describe('GET /api/auth/youtube/callback', () => {
  ...
  it('redirects with token_exchange_failed when fetch fails', async () => {
    vi.mocked(getUser).mockResolvedValue({ id: 'user-1' } as any)
    vi.mocked(global.fetch as any).mockResolvedValue({ ok: false, status: 400, text: vi.fn().mockResolvedValue('error') })
    const res = await GET(req({ code: 'c', state: 's' }))
    expect(res.status).toBe(307)
