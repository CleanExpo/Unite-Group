...
export async function POST(request: Request) {
  ...
  try {
    body = (await request.json()) as PiRouteRequestBody
  } catch {
  ...
  return NextResponse.json({
    ...
    machineAssignment,
    receipt: {
      id: `receipt_${routeResult.taskPacket.id}`,
      status: 'routed',
      generatedAt: routeResult.contextPack.updatedAt,
      source: '/api/pi/route',
