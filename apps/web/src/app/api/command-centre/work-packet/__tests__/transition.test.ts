...
function makeRequest(body: unknown): Request {
  return new Request('https://app.test/api/command-centre/work-packet/wp-test-2026/transition', {
    method: 'POST',
...
describe('POST /api/command-centre/work-packet/[id]/transition', () => {
  ...
  it('applies a valid transition and returns the new packet', async () => {
    ...
    const next = makePacket({ status: 'completed', evidencePath: '/evidence/x.md' })
    const result: ApplyPacketTransitionResult = { ok: true, packet: next, reason: 'completed' }
