...
// Keep the real BUSINESS_TO_TEAM map (used by the route's dry-run Linear
// mapping); only the network-touching createIssue is stubbed.
vi.mock('@/lib/integrations/linear', async (importOriginal) => {
  ...
  return { ...actual, createIssue: vi.fn() }
})

import { getUser } from '@/lib/supabase/server'
import { saveWorkPacket, listWorkPackets } from '@/lib/command-centre/work-packet-store'
...
function makePostRequest(body: unknown): Request {
  return new Request('https://app.test/api/command-center/work-packet', {
    method: 'POST',
...
function makeGetRequest(query: Record<string, string> = {}): Request {
  const url = new URL('https://app.test/api/command-center/work-packet')
...
describe('POST /api/command-center/work-packet', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('persists via saveWorkPacket and returns the packet, mode, linearInput and created', async () => {
    ...
    // No live Linear create without CC_LINEAR_LIVE — the route runs dry-run.
...
describe('GET /api/command-center/work-packet', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
