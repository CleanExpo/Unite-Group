...
import { AUTONOMOUS_LABELS } from '@/lib/command-centre/linear-claim'
...
export async function POST(
  ...
  const evidenceIds = (body.evidenceIds ?? []).map((e) => e.trim()).filter(Boolean)
  ...
  // Build a claim-eligible description: the "## Acceptance Criteria" heading is
  // required for the autonomous claim loop (src/lib/command-centre/linear-claim.ts).
  ...
  // Opt-in autonomy: only label the issue auto-claimable when the founder asks.
  const labelNames = body.autonomous ? [AUTONOMOUS_LABELS[0]] : []
