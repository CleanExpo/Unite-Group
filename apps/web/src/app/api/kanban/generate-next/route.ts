...
interface GeneratedTask { title: string; context: string }
...
function parseTasks(text: string): GeneratedTask[] {
  ...
  try {
    ...
    return arr
      .filter((t): t is GeneratedTask => Boolean(t) && typeof (t as GeneratedTask).title === 'string' && (t as GeneratedTask).title.trim().length > 0)
      .map((t) => ({ title: String(t.title).trim().slice(0, 200), context: String(t.context ?? '').trim().slice(0, 600) }))
      .slice(0, 5)
  } catch {
    return []
  }
...
export async function POST(request: Request) {
  ...
  const prompt = [
    ...
    '',
    'Propose 3–5 high-leverage, specific, non-duplicate tasks for THIS project at THIS stage. Each must be independently actionable by an autonomous coding agent: a clear scope and a verifiable done-condition.',
    'Return ONLY a JSON array (no prose, no markdown fences): [{"title":"…","context":"1–2 sentences: scope + how we know it is done"}]',
  ].join('\n')
