      ...
      <div className="bg-[#fff7ec] border border-white/6 rounded-sm p-4 flex flex-col gap-4">
        {[0, 1, 2, 3].map((i) => (
          <div key={i} className="flex flex-col gap-2">
            <div className="h-2 w-24 bg-white/6 rounded-sm animate-pulse" />
            <div className="h-8 w-full bg-white/6 rounded-sm animate-pulse" />
          </div>
        ))}
        <div className="h-8 w-32 bg-white/6 rounded-sm animate-pulse mt-2" />
      </div>
    </div>
  )
}
