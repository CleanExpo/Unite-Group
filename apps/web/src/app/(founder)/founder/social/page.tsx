...
export default async function SocialPage({
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {params.connected && (
        <div className="text-xs text-[#2f9e44]/80 border border-[#2f9e44]/20 bg-[#2f9e44]/5 px-4 py-2.5 rounded-sm">
      ...
      <div className="flex items-center gap-4 text-sm">
        <div className="border border-white/[0.08] px-4 py-2 rounded-sm" style={{ background: 'var(--surface-card)' }}>
          ...
          <span className="text-[#2f9e44]">{configuredCount}/{platforms.length}</span>
        </div>
        <div className="border border-white/[0.08] px-4 py-2 rounded-sm" style={{ background: 'var(--surface-card)' }}>
          <span className="text-muted">Connected:</span>{' '}
          <span className="text-[#2f9e44]">{connectedCount}/{platforms.length}</span>
        </div>
      </div>
          ...
          >
            <div className="flex items-center gap-4">
              <div
                ...
                style={{
                  ...
                  color: platform.connected ? '#2f9e44' : 'var(--color-text-muted)',
                  border: `1px solid ${platform.connected ? 'rgba(47, 158, 68, 0.2)' : 'rgba(255,255,255,0.08)'}`,
                }}
              >
                {platform.icon}
              </div>
              <div>
                ...
                {platform.connected && platform.connectedAt && (
                  <p className="text-[10px] mt-1 text-[#2f9e44]/70">
            ...
            <div className="flex items-center gap-3">
              {!platform.configured ? (
                ...
                <span className="text-[10px] uppercase tracking-widest text-[#2f9e44]/80 border border-[#2f9e44]/30 px-2.5 py-1 rounded-sm">
                  Live
                </span>
              ) : (
                <a
        ...
        <div className="space-y-2">
          {platforms.map((platform) => (
            <div key={platform.key} className="flex items-center gap-2 text-xs">
              ...
              />
              <span style={{ color: platform.connected ? '#2f9e44' : 'var(--color-text-muted)' }}>
                {platform.name}
              </span>
