...
describe('command centre operator gateway UI source', () => {
  ...
  it('shows the multi-CLI runtime monitor without enabling dispatch or shared credentials', () => {
    ...
    expect(source).toContain('/api/hermes/operator-gateway/runner-telemetry')
    expect(source).toContain('Registry view only')
    expect(source).toContain('Claude Code, MiniMax, and Cursor remain install/login blocked')
    expect(source).not.toContain('Claude Max builders and MiniMax run in isolated monitors')
    expect(source).toContain('Telemetry contract')
    expect(source).toContain('Telemetry monitors')
    expect(source).toContain('Telemetry connected')
    expect(source).toContain('Next gate')
  ...
  it('shows mobile voice intake from Plaud to the second brain without auto-dispatch', () => {
    ...
    expect(source).toContain('Source note writing enabled')
    expect(source).toContain('Board packet generation enabled')
