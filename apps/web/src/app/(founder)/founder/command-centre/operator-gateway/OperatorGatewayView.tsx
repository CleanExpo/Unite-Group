import type { CommandCentreOperatorSurfaceView } from '@/lib/operator-gateway/command-centre'
import type { OperatorEvent, OperatorJobsView } from '@/lib/operator-gateway/jobs'
import type { GatewayConnection } from '@/lib/operator-gateway/presence'
import {
  Card,
  CollapsibleGroup,
  MetricCard,
...
export interface OperatorGatewayViewProps { view: CommandCentreOperatorSurfaceView | null; jobsView?: OperatorJobsView; jobEvents: OperatorEvent[]; agentConnection: GatewayConnection | null; className?: string }

export function OperatorGatewayView({ view, jobsView, jobEvents, agentConnection, className }: OperatorGatewayViewProps) {
  if (!view) {
    return (
      <MissionControlShell section="operator-gateway" title="Operator gateway" className={className}>
        <Card aria-label="operator sources unavailable">
          <h2>Operator sources unavailable</h2>
          <p role="status">Connect the authenticated operator sources to inspect execution lanes, jobs, agent heartbeats and delivery evidence.</p>
          <p>Connection and gate status have not been checked in this environment. No job controls are available until those sources can be read.</p>
        </Card>
      </MissionControlShell>
    )
  }
