// src/app/(founder)/founder/command-centre/BlockedLanesTile.test.ts
//
...
describe('BlockedLanesTile source contract', () => {
  ...
  it('renders the data-testid hook used by the deck selector', () => {
    expect(src).toContain('data-testid="blocked-lanes-tile"')
  })

  it('renders the data-testid empty-state hook when no lanes are blocked', () => {
    expect(src).toContain('data-testid="blocked-lanes-tile-empty"')
  })

  it('renders the data-testid error-state hook when the file cannot be read', () => {
    expect(src).toContain('data-testid="blocked-lanes-tile-error"')
  })

  it('exposes a server-side loader that NEVER throws (returns structured result)', () => {
    expect(src).toContain('export async function loadBlockedLanesData')
    expect(src).toContain('read_error')
    expect(src).toMatch(/catch\s*\(\s*err\s*:\s*unknown\s*\)/)
  })

  it('exposes the isBlocked helper for testability', () => {
    expect(src).toContain('export function isBlocked')
  })
