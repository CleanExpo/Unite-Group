...
import type { PipelineBoardData } from '@/lib/command-centre/pipeline-opportunities'
...
export interface PortfolioViewProps { projects: CommandCentreProject[]; integrationStatuses: ProjectIntegrationStatus[]; pipeline: PipelineBoardData; integrationStatus?: React.ReactNode; businessFocus?: React.ReactNode; registryUnavailable?: string; className?: string }

export function PortfolioView({ projects, integrationStatuses, pipeline, integrationStatus, businessFocus, registryUnavailable, className }: PortfolioViewProps) {
  const activeCount = projects.filter((project) => project.status === 'active').length
  return (
    <MissionControlShell section="portfolio" title="Businesses & projects" className={className}>
      ...
      <div className={`${shell.canvasScope} ${shell.glassSectionHead}`} id="portfolio">
        ...
        <span className={shell.glassSub}>{registryUnavailable ?? `${projects.length} units · ${activeCount} active`}</span>
      ...
      <section className={`${styles.reveal}`} style={{ animationDelay: '0.08s' }}>
        {businessFocus ?? <p>Business focus source unavailable in this environment.</p>}
      </section>
