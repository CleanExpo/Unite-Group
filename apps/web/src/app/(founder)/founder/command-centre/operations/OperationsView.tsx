...
export interface OperationsViewProps {
  dashboard: DashboardSummaryResult;
  evidence: EvidenceStreamResult;
  actionQueue: ActionQueueTileData;
  blockedLanes: BlockedLanesData;
  crmAutonomy?: React.ReactNode;
  agentEventsWall: AgentEventsWallResult;
  serverDataUnavailable?: string;
  className?: string;
}
    ...
    >
      ...
      >
        ...
        <span className={shell.glassSub}>
          approval → lifecycle gate → operator job · dispatch is Board-gated
        </span>
      </div>
      ...
      >
        {crmAutonomy ?? (
          <p>
            Authenticated execution policy and recent jobs are unavailable in
            this environment.
          </p>
        )}
      </section>
      ...
      {/* Local-only, read-only portfolio drift and independent-worker projection. */}
      <div
        className={`${shell.canvasScope} ${shell.glassSectionHead}`}
        id="agent-fleet"
      >
        <h2>Portfolio control plane</h2>
        <span className={shell.glassSub}>
          repository drift · device freshness · durable checkpoints
        </span>
      </div>
      ...
      <div
        className={`${shell.canvasScope} ${shell.glassSectionHead}`}
        id="agent-events-wall"
      >
        <h2>Agent Events Wall</h2>
        <span className={shell.glassSub}>
          runner lifecycle · claimed → started → draft PR · heartbeats
        </span>
        {agentEventsWall.source === 'connected' && (
          <span className={shell.glassSrc}>
            last {agentEventsWall.events.length} events
          </span>
        )}
      </div>
      ...
      >
        ...
        <span className={shell.glassSrc}>
          {serverDataUnavailable
            ? 'Source unavailable'
            : `${dashboard.entries.length} sources · ${dashboard.red_count} red · ${dashboard.amber_count} amber · ${dashboard.green_count} green · ${dashboard.error_count} errors`}
        </span>
      </div>
      ...
      >
        ...
        <span className={shell.glassSub}>
          receipts are first-class · every action leaves a trail
        </span>
        <span className={shell.glassSrc}>
          {serverDataUnavailable
            ? 'Source unavailable'
      ...
      >
        ...
        <span className={shell.glassSub}>
          via <code style={{ fontSize: '0.7rem' }}>GitHub API</code>
        </span>
      </div>
      ...
      >
        ...
        <span className={shell.glassSub}>
          Margot state · contractor activity · email roster
        </span>
      </div>

      <section
        className={`${styles.integrationGrid} ${styles.reveal}`}
