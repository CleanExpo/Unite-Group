 ...
 * `next` is the top-priority open issue in that stage. Open work the map
 * cannot classify is never Done: with no classified word it throws.
 */
export function summariseTeam(
  ...
  if (active === null && issues.length > 0) {
    throw new Error(`${key}: ${issues.length} open issue(s) in states the stage map does not know (${[...unmapped].sort().join(', ')})`)
  }
...
async function fetchTeamOpenIssues(
  ...
  // More pages remain after the cap: an unseen issue could be further along than
  // every one read, so no stage is established. Fail rather than publish a guess.
  return { ok: false, error: `${teamKey}: more than ${MAX_PAGES * PAGE_SIZE} open issues: the stage cannot be verified from a partial read` }
}
...
export async function fetchTeamStages(deps: ProjectStagesDeps = {}): Promise<TeamStagesResult> {
  ...
  for (const t of teams.teams) {
    ...
    // An open issue the map cannot classify has an unknown stage: it could be the
    // team's furthest-along work, and alone it would read as Done. Fail the read.
    const unknown = [...new Set(fetched.issues.filter((i) => classifyIssue(i) === null).map((i) => i.stateName))].sort()
    if (unknown.length > 0) {
      return { ok: false, error: `${t.key}: open issues in states the stage map does not know (${unknown.join(', ')}); add them to data/command-centre/stage-map.json` }
    }
