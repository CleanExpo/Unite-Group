...
// project / task batch is complete, builds a final summary, and (live only)
// moves it to a closed state — while refusing to close broad roadmap projects,
// surfacing open blockers that prevent closure, and grouping stale duplicates.
//
// SAFETY: DRY-RUN by default. A live closure happens only when opts.live === true
// AND process.env.CC_LINEAR_LIVE === '1' (same double-gate as the claim loop).
// Pure assessment logic is dependency-free and fully unit-tested.
...
export interface CleanupAssessment {
  ...
  state: CleanupState
  closeable: boolean
  counts: { total: number; terminal: number; open: number; blocked: number; accepted: number }
  blockers: { identifier: string; reason: string }[]
  duplicates: DuplicateGroup[]
}

export function classifyIssue(issue: CleanupIssue): IssueClass {
  if (issue.stateType === 'completed' || issue.stateType === 'canceled') return 'terminal'
  if (issue.blockedByOpenCount > 0 || issue.labels.some((l) => l.toLowerCase().startsWith(BLOCKED_LABEL_PREFIX))) {
    return 'blocked'
  }
  return 'open'
}
...
export function assessBatch(batch: CleanupBatch): CleanupAssessment {
  ...
  if (batch.scopeKind === 'roadmap') {
    return { ...base, state: 'not_scoped', closeable: false, counts: tally(batch, accepted), blockers: [] }
  }
  if (batch.alreadyClosed) {
    return { ...base, state: 'already_closed', closeable: false, counts: tally(batch, accepted), blockers: [] }
  }
  ...
  const blockers = batch.issues
    ...
    .map((i) => ({
      ...
      reason: i.labels.find((l) => l.toLowerCase().startsWith(BLOCKED_LABEL_PREFIX)) ?? 'blocked by open dependency',
    }))

  // Non-terminal issues that are NOT accepted exclusions.
  const unresolved = batch.issues.filter((i) => classifyIssue(i) !== 'terminal' && !accepted.has(i.id))

  let state: CleanupState
  if (unresolved.length === 0) {
    state = counts.accepted > 0 && counts.terminal < counts.total ? 'closeable_with_exclusions' : 'complete'
  ...
  return { ...base, state, closeable: state === 'complete' || state === 'closeable_with_exclusions', counts, blockers }
}
...
function tally(batch: CleanupBatch, accepted: Set<string>) {
  ...
  return { total: batch.issues.length, terminal, open, blocked, accepted: accepted.size }
}
...
export function buildCleanupSummary(assessment: CleanupAssessment, batch: CleanupBatch, input: CleanupSummaryInput): string {
  const done = batch.issues.filter((i) => classifyIssue(i) === 'terminal').map((i) => i.identifier)
  const idToIdent = new Map(batch.issues.map((i) => [i.id, i.identifier]))
  const excluded = (batch.acceptedExclusions ?? []).map((id) => idToIdent.get(id) ?? id)
  const lines = [
  ...
  if (excluded.length) lines.push(`- Accepted exclusions: ${excluded.join(', ')}`)
  if (assessment.blockers.length) lines.push(`- Open blockers (kept open): ${assessment.blockers.map((b) => b.identifier).join(', ')}`)
  if (assessment.duplicates.length) {
    lines.push(`- Duplicate clusters flagged for review: ${assessment.duplicates.map((d) => d.identifiers.join('/')).join('; ')}`)
  }
  lines.push('', 'Next: review the flagged duplicates; broad roadmap projects are left untouched.')
  return lines.join('\n')
}

// ─── Orchestration (DI, double-gated) ───────────────────────────────────────

export interface CleanupDeps {
  /** Close/complete the scoped Linear project or task batch. */
  closeScope: (scopeId: string) => Promise<void>
  /** Post the final summary comment to the scope. */
  postSummary: (scopeId: string, body: string) => Promise<void>
}
...
export async function runCleanup(batch: CleanupBatch, deps: CleanupDeps, opts: CleanupOptions = {}): Promise<CleanupResult> {
  ...
  if (!assessment.closeable) {
    return { ran_at: ranAt, mode: isLive ? 'live' : 'dry-run', state: assessment.state, closed: false, summary: null, assessment }
  }

  const summary = buildCleanupSummary(assessment, batch, { runner, at: ranAt })

  if (!isLive) {
    return { ran_at: ranAt, mode: 'dry-run', state: assessment.state, closed: false, summary, assessment }
  }

  await deps.postSummary(batch.scopeId, summary)
  await deps.closeScope(batch.scopeId)
  return { ran_at: ranAt, mode: 'live', state: assessment.state, closed: true, summary, assessment }
}
