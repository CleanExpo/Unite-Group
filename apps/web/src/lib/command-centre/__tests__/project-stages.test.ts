...
describe('summariseTeam', () => {
  ...
  it('is Done only with nothing open', () => {
    expect(summariseTeam('DR', 'DR', [], false).stage).toBe('Done')
  })
  it('never reads Done while an open issue sits in an unmapped state (P1-UNMAPPED-OPEN-WORK-READS-DONE)', () => {
    expect(() => summariseTeam('DR', 'DR', [issue({ stateName: 'Limbo', stateType: 'mystery' })], false)).toThrow(
      /DR: 1 open issue\(s\) in states the stage map does not know \(Limbo\)/,
    )
  })
})
...
describe('fetchTeamStages', () => {
  ...
  it('fails the whole read at the issue page cap — unseen issues beyond it may be further along (P1-CAPPED-ISSUES-PUBLISH-UNVERIFIED-STAGE)', async () => {
    // Four pages of Planning, then a fifth page of Production the capped read never sees.
    const pages = [
      ...Array.from({ length: 4 }, (_, p) => Array.from({ length: 3 }, (_, i) => rawNode(`RA-${p}-${i}`, 'Todo', 'unstarted'))),
      [rawNode('RA-late', 'In Review', 'started')],
    ]
    const fetchImpl = linearMock({ RA: pages })
    expect(await fetchTeamStages({ apiKey: 'k', fetchImpl })).toEqual({
      ok: false,
      error: 'RA: more than 1000 open issues: the stage cannot be verified from a partial read',
    })
  })
  it('the CLI refuses the same capped read', async () => {
    const cli = await import(pathToFileURL(path.resolve(process.cwd(), '..', '..', 'scripts', 'stage.mjs')).href)
    const pages = [
      ...Array.from({ length: 4 }, (_, p) => Array.from({ length: 3 }, (_, i) => rawNode(`RA-${p}-${i}`, 'Todo', 'unstarted'))),
      [rawNode('RA-late', 'In Review', 'started')],
    ]
    const fetchImpl = linearMock({ RA: pages }) as unknown as typeof fetch
    await expect(cli.fetchTeamStages({ apiKey: 'k', fetchImpl, map: STAGE_MAP })).rejects.toThrow(
      'RA: more than 1000 open issues: the stage cannot be verified from a partial read',
    )
  })
  it('still publishes a stage when the last page arrives exactly at the cap', async () => {
    const pages = Array.from({ length: 4 }, (_, p) => [rawNode(`RA-${p}`, 'Todo', 'unstarted')])
    const result = await fetchTeamStages({ apiKey: 'k', fetchImpl: linearMock({ RA: pages }) })
    if (result.ok !== true) throw new Error(`expected ok, got ${JSON.stringify(result)}`)
    expect(result.teams[0]).toMatchObject({ stage: 'Planning', open: 4, capped: false })
  })
...
// ─── Review round 5 (codex, head 855139b44): one P1, watched red first ───

describe('fetchTeamStages — review round 5 finding (P1-UNMAPPED-OPEN-WORK-READS-DONE)', () => {
  const UNKNOWN = 'RA: open issues in states the stage map does not know (New workflow state); add them to data/command-centre/stage-map.json'

  it('fails the whole read when the only open issue is in an unmapped state — never Done', async () => {
    const fetchImpl = linearMock({ RA: [[rawNode('RA-999', 'New workflow state', 'mystery')]] })
    expect(await fetchTeamStages({ apiKey: 'k', fetchImpl })).toEqual({ ok: false, error: UNKNOWN })
  })

  it('fails the whole read when an unmapped open issue sits beside mapped ones — its stage is unknown', async () => {
    const fetchImpl = linearMock({ RA: [[rawNode('RA-1', 'Backlog', 'backlog'), rawNode('RA-999', 'New workflow state', 'mystery')]] })
    expect(await fetchTeamStages({ apiKey: 'k', fetchImpl })).toEqual({ ok: false, error: UNKNOWN })
  })

  it('the CLI refuses the same read, and its summariseTeam never returns Done for open work', async () => {
    const cli = await import(pathToFileURL(path.resolve(process.cwd(), '..', '..', 'scripts', 'stage.mjs')).href)
    const fetchImpl = linearMock({ RA: [[rawNode('RA-999', 'New workflow state', 'mystery')]] }) as unknown as typeof fetch
    await expect(cli.fetchTeamStages({ apiKey: 'k', fetchImpl, map: STAGE_MAP })).rejects.toThrow(UNKNOWN)
    const lone = [issue({ stateName: 'New workflow state', stateType: 'mystery' })]
    expect(() => cli.summariseTeam('RA', 'RA', lone, false, STAGE_MAP)).toThrow(/RA: 1 open issue\(s\) in states the stage map does not know/)
  })
})
