...
// Review round 5 (codex, head 855139b44): the tests above never reached the
// malformed-line guard, so disabling it left them green. Each case below is a
// ledger the parser can only half-read; without the guard it would render as
// ok:true with the unreadable row silently missing.
describe('evaluateFounderQueue fails closed on structurally malformed ledgers (P0-MALFORMED-LEDGER-GUARD-TEST-SURVIVES-MUTANT)', () => {
  const good = `| F1 | Decide | 2026-08-16 | — | x | y | open |`
  it.each([
    ['a row with the wrong width', ledger(`${good}\n| F2 | Click Connect Google | 2026-07-06 | — | UNI-2329 | open |`), /has 6 cells, expected exactly 7/],
    ['a row drifted out of its table body', `${ledger(good)}\n| F9 | drifted | 2026-08-01 | — | x | y | open |\n`, /outside any table body/],
    ['an Open header with no separator', `## Open\n\n| ID | Decision | Opened | Age (days) | Blocks | Context | Status |\n`, /no separator beneath it/],
    ['no Open header at all', '# nothing here\n', /Open table header was never found/],
  ])('%s → ok:false, no rows', (_name, raw, pattern) => {
    const result = evaluateFounderQueue(raw, 'fixture', NOW)
    expect(result.ok).toBe(false)
    expect(result).not.toHaveProperty('rows')
    if (result.ok) return
    expect(result.error).toMatch(/unreadable line\(s\)/)
    expect(result.error).toMatch(pattern)
  })
})
