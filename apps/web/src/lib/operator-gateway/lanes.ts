...
export const OPERATOR_LANES: readonly OperatorLane[] = [
  ...
  {
    laneId: 'claude_code_max_tertiary',
    provider: 'anthropic',
    tool: 'claude code',
    authMode: 'not_installed',
    maxPlanBased: true,
    ...
    status: 'design_only',
    lastVerifiedAt: null,
    notes: 'Tertiary Claude Code lane for the third Claude Max plan allocation. Design-only until installed and logged in as its own operator session.',
  },
  {
  ...
  },
  {
    ...
    maxPlanBased: true,
    apiKeyRequired: false,
    allowedTaskTypes: ['video_media', 'speech', 'music', 'image_generation', 'creative_asset_generation', 'coding_assist'],
    prohibitedTaskTypes: [...HARD_GATED_TASKS, 'public_publishing', 'client_delivery_without_review'],
    externalActionAllowed: false,
    productionActionAllowed: false,
    requiresHumanSession: true,
    evidenceRequired: ['summary', 'asset_manifest', 'quota_snapshot', 'audit_record'],
    status: 'design_only',
    lastVerifiedAt: null,
    notes: 'MiniMax CLI lane for MiniMax Max/Token Plan capabilities. Design-only until installed and authenticated by the operator; no CRM-stored API key.',
  },
  {
    laneId: 'hermes_local',
    provider: 'hermes',
