...
3. Contacts
   ...
   - Next: validate the migration on a Supabase database branch (never against prod) before any promotion, then add server-route/mocked tests for contact creation/linking. Promote to prod only by merging an approved branch — never apply to prod directly or autonomously.
...
4. Opportunities
   ...
   - Next: validate the migration on a Supabase database branch (never against prod) before any promotion, then add server-route/mocked tests for opportunity drafts. Promote to prod only by merging an approved branch — never apply to prod directly or autonomously.
...
Branch-first DB rule:

- Any schema change must be validated on a Supabase database branch (an ephemeral per-branch DB) before production promotion. Never validate against prod (`lksfwktwtmyznckodsau`).
- Promote to prod only by merging an approved branch — never apply migrations to prod directly or autonomously.
- Do not apply migrations to production from this inventory task. (The mirror sandbox project `xgqwfwqumliuguzhshwv` and `scripts/sandbox-wizard.sh` were retired on 2026-06-15; canonical rules live in `CLAUDE.md` / `apps/empire/CLAUDE.md`.)
