...
| Founder opportunities read surface | `src/app/api/founder/opportunities/route.ts`; `src/components/founder/opportunities/OpportunitiesPageClient.tsx` | `src/app/api/founder/opportunities/__tests__/route.test.ts`; `src/components/founder/opportunities/__tests__/OpportunitiesPageClient.test.tsx` | Founder-scoped `crm_opportunities` read; pipeline summary; explicit source-of-truth metadata showing `crm_opportunities` as CRM source, forecast-only mode, and Stripe as billing truth; UI renders those labels and keeps error state honest instead of fake-empty. | Covered with local API/component tests; no browser write path or billing authority. | Add pagination and stale-source/readiness labelling before larger opportunity queues. |
