...
38. **Re-run Google consent so a token actually persists, then run the live Gmail import from the browser**
   - Status: the live Gmail import is still **UNKNOWN / HUMAN-GATED**. Despite a reported consent click, the prod credentials vault shows **no google-service write on 05/07/2026** (`credentials_vault` `service='google'`: `count=6`, newest `created_at` 14/03, `updated_at` 28/06, `last_accessed_at` 08/05 — Supabase MCP, timestamps only) [VERIFIED]. The OAuth callback only writes on a successful token exchange (`src/app/api/auth/google/callback/route.ts:91-105`), so consent did not complete server-side.
   - Why the agent cannot finish this autonomously [VERIFIED]: `POST /api/email/contacts/import` requires a founder session (`route.ts:186` `getUser()`) and `/api/email` is not a `PUBLIC_PATH` in `src/proxy.ts`, so `/api/*` is gated by `hasPrivateAccess`. There is **no `CRON_SECRET`/bearer lane** on this route. The agent holds no founder session, so the import is a browser-only action by design — this is correct security posture (No-Invaders #1/#6), not a gap to "fix" by adding a bypass lane.
   - Exact human steps: (1) log in to prod as founder; (2) Settings → Integrations — Email Accounts → **Connect Google** (or open `/api/auth/google/authorize?email=<your-gmail>` directly); (3) approve consent and confirm the redirect is `/founder/email?connected=<email>` (an `?error=…` redirect means it failed — common causes: `redirect_uri` mismatch vs `GOOGLE_CALLBACK_URL`, or the connect UI #630 not yet deployed to the alias you used); (4) from `/founder/email`, trigger the tagged live import. A fresh 05/07 `credentials_vault` google row is the objective "consent persisted" signal.
   - Note on multiple accounts: six google rows already exist, so with a fresh connect `resolveGoogleAccount` will see >1 account and return `400 gmail_account_required` unless `accountEmail` is supplied — the import call must pass the specific `accountEmail` to disambiguate.
   - Decision requested: confirm whether (a) the founder will complete the browser consent + import (closes the Gmail half), or (b) this ticket's Gmail live proof is deferred and re-recorded as human-gated. Either way the agent-side work is complete: routes, guards, and the connect UI (#630) are in place; only the founder-session actions remain.
