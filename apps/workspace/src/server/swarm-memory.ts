...
export const SWARM_SHARED_MEMORY_ROOT = join(
  homedir(),
  '.openclaw',
  'workspace',
  'memory',
...
export async function getTieredContext(
  ...
  if (activeMissionId) {
    const summaryPath = join(
      swarmWorkerMissionMemoryRoot(agentId, activeMissionId),
      'SUMMARY.md',
    )
    const missionSummary = readTextIfExists(summaryPath)
    if (missionSummary) {
      sections.push(
        `## Active mission (${activeMissionId})\n\n` + missionSummary.trim(),
      )
    }
  }
  ...
  if (existsSync(episodesRoot)) {
    ...
    if (episodicLines.length) {
      sections.push(
        '## Recent episodic entries (last 10)\n\n' + episodicLines.join('\n\n'),
      )
    }
  }
