...
export const MISSION_CONTROL_LAYOUT_CONTRACT = {
  ...
  guardrails: ['no-new-vendors', 'no-sakana-fugu-logic'],
} as const

export const MISSION_CONTROL_DOMAIN_CARDS = [
  {
    title: 'Memory Galaxy',
    subtitle: 'Obsidian + retrieval spine',
    actions: ['Daily briefing', 'Vault context map'],
  },
  {
    title: 'Hermes Jarvis',
    subtitle: 'Voice + command routing',
    actions: ['Voice wake mode', 'Route mission'],
  },
  {
    title: 'News Radar',
    subtitle: 'Signals + briefings',
    actions: ['Scan feeds', 'Draft watchlist'],
  },
  {
    title: 'Video Agent',
    subtitle: 'Video ops cockpit',
    actions: ['Video Command Center', 'Render status'],
  },
  {
    title: 'SEO Agent OS',
    subtitle: 'Keyword → draft → approval',
    actions: ['Generate drafts', 'Approval queue'],
  },
  {
    title: 'Loop Engineering',
    subtitle: 'Agent improvement loops',
    actions: ['Run diagnostics', 'Queue repair'],
  },
] as const
...
function getLastAssistantMessage(
  ...
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const message = messages[index] as HistoryMessage | undefined
      ...
      try {
        ...
        setDirectoryBrowserEntries(
          Array.isArray(data.entries)
            ? data.entries.filter(
                (entry: unknown): entry is FileBrowserEntry =>
                  typeof entry === 'object' &&
                  entry !== null &&
                  (entry as FileBrowserEntry).type === 'folder',
              )
            : [],
        )
      } catch (error) {
