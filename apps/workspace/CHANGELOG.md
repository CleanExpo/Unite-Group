...
- **`docker compose up` now pulls pre-built images by default** (#82) — the
  gateway and UI retain human-readable `latest` tags but execute only through
  the reviewed multi-platform `tag@sha256` references in `docker-compose.yml`.
  Agent state persists in the `claude-data` named volume. Adds
  `docker-compose.dev.yml` overlay for building from source; digest refreshes
  follow `docs/container-image-digest-refresh.md` from the monorepo root.
