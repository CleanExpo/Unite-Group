...
- Immutable identity: `CleanExpo/Unite-Group` PR `903`; base `2993429d6a812747550bac82f33568417c8b8279`; repaired dependency target head `ce065adaf0f057837a1b1ec4721bf4d95f771925`; governed scope exactly `apps/web/package.json`, `apps/web/pnpm-lock.yaml`, `apps/workspace/package.json`, `apps/workspace/pnpm-lock.yaml`, `packages/pi-ceo-operator-mcp/package-lock.json`, `packages/pi-ceo-operator-mcp/package.json`.
...
3. **M03 — Read live PR identity and fail stale before runtime** (L0; gate `G-HARDSTOP-CLEARANCE`) — Verify: Live repository, PR, base, head and exactly two changed paths equal immutable identity; mismatch returns STALE before checkout/install/build/mount/launch. Planned command receipts: C03. Failure: `STALE` absorbing, run-owned cleanup only.
